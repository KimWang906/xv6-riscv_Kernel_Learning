...
자식 프로세스는 **wc** 프로그램을 실행하기 위해 **execvp()** System Call을 호출한다.  
**wc** 프로그램은 단어의 개수를 세는 프로그램이다.  
사실 예제 프로그램은 자신의 소스 파일을 인자로 하여 **wc**를 실행하고  
소스 코드의 행 개수, 단어의 개수, 바이트의 개수를 알려 준다.  

사실 **fork()** System Call만큼 **exec()** System Call도 이해하기 어렵다.  
**exec()** System Call은 다음과 같은 과정으로 수행된다.  
